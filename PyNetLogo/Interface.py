...
def get_patch(x, y):
    world_name = threading.current_thread().name
    return [worlds[world_name].get_patch(x*worlds[world_name].patches_size[0], y*worlds[world_name].patches_size[1])]
...
###
def create_world(row_patches_width, col_patches_width, max_px, max_py, world_name=""): 
    import Datas      
    world                   = World([row_patches_width, col_patches_width], [max_px*2 + 1, max_py*2 + 1], world_name)
    worlds[world.name]      = world
    worlds_locks[world.name]= threading.Lock()

    worlds_display_position.append(Datas.Globals.max_x)
    Datas.Globals.max_x += world.patches_size[0] * world.patches_number[0] + 5 # 5 : border between worlds
    if world.patches_size[1] * world.patches_number[1] > Datas.Globals.max_y :
        Datas.Globals.max_y = world.patches_size[1] * world.patches_number[1]

...
class Turtle(Agent) :
    ...
    def __init__(self, world_name, actions=[]):
        ...
        self.patch              = world.get_patch(self._x, self._y)
        self.patch.turtles_here.append(self)
        
    ###
    #    Draw the turtle on the window in parameter
    ...
    ###
    def _torus_coordinates(cls, x, y):
        from Datas.Globals  import max_pxcor, max_pycor
    ...
    ###
    def _calc_coordinates(cls, x, y, angle, distance):
        from Datas.Globals  import world
              
        # Forward 1 = movement of 1 patch = 1 * patchSize
        distance_x_patches = distance * world.patches_size[0]
        distance_y_patches = distance * world.patches_size[1]

        x1 = x + distance_x_patches * math.cos(math.radians(angle))
        y1 = y - distance_y_patches * math.sin(math.radians(angle))
        
        x1, y1 = Turtle._torus_coordinates(x1, y1)
    ...
    def _move(self, distance):
        ...
        self._x, self._y = Turtle._calc_coordinates(x0, y0, self.heading, distance)
        ...
        if self.pen_mode == True :
            world.lines_list.append((self, (x0, y0), (self._x, self._y)))
            world.updated_lines.append((self, (x0, y0), (self._x, self._y)))
    ...
    def hatch(self, nb_sons=1, actions=[]):
        ...
        for _ in range(0, nb_sons) :
            ...
            world.temp_new_turtles_list.append(new_turtle)  # @UndefinedVariable
    ...
    def die(self):
        ...
        self.patch.turtles_here.remove(self)
        world.temp_del_turtles_list.append(self)  
    ...
    ###  
    def pixels_coordinates(cls, attribute_name, attribute_value) :
        from Datas.Globals  import world
        
        if attribute_name == "x" :
            center = (world.patches_number[0] - 1) // 2
            value = (attribute_value  + center) * world.patches_size[0]
        elif attribute_name == "y" :
            center = (world.patches_number[1] - 1) // 2
            value = (world.patches_number[1] - 1 - attribute_value - center)* world.patches_size[1]  
    ...
    ###    
    def patches_coordinates(cls, attribute_name, attribute_value) :
        from Datas.Globals  import world
        
        if attribute_name == "x" :
            center = (world.patches_number[0] - 1) // 2
            value = (attribute_value/world.patches_size[0]) - center
            
        elif attribute_name == "y" :
            center = (world.patches_number[1] - 1) // 2
            value  = -(attribute_value/world.patches_size[1]) - center - 1 + world.patches_number[1]
    ...
    def set(self, attribute_name, attribute_value, rand=""):
        from Datas.Globals  import max_px, max_py
        ...
        if attribute_name == "xcor" :
            ...
            setattr(self, "_x", Turtle.pixels_coordinates("x", value))
        elif attribute_name == "ycor" :
            if rand != "" :
                value = random.randrange(-max_py, max_py, 1)
            setattr(self, "ycor", value)   
            setattr(self, "_y", Turtle.pixels_coordinates("y", value))
        else :
    ...
    def __getattribute__(self, attribute_name): 
        ...
        if attribute_name == "xcor" :
            self.xcor = Turtle.patches_coordinates("x", self._x)
        elif attribute_name == "ycor" :
            self.ycor = Turtle.patches_coordinates("y", self._y)
    ...
    def patch_right_and_ahead(self, angle, distance):
        ...
        x0, y0 = self._x, self._y      
        x0, y0 = Turtle._calc_coordinates(x0, y0, new_angle, distance)

        patch = world.get_patch(x0, y0)

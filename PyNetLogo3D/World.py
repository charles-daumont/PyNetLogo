...
class World(threading.Thread):
    ...
    def generate_patches_grid(self):
        ...
        center_z = (self.patches_number[2] -1) // 2
        
        for x_cor in range(self.patches_number[0]) :
            # Convert coordinates 0,0 bottom left corner to 0,0 center
            x = x_cor - center_x
            
            for y_cor in range(self.patches_number[1]) :
                y = y_cor - center_y
                
                for z_cor in range(self.patches_number[2]) :
                    z = z_cor - center_z
                    new_patch = Patch(x, y, z, x_cor * self.patches_size, y_cor * self.patches_size, z_cor * self.patches_size, self.patches_size, None, self.name)
    ...
    def get_patch(self, x, y, z):
        ...
        pos_x = x // self.patches_size
        pos_y = y // self.patches_size
        pos_z = z // self.patches_size

        patch_pos = pos_x*self.patches_number[1]*self.patches_number[2] + pos_y*self.patches_number[2] + pos_z
